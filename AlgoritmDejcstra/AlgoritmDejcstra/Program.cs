...
class Program
{
    ...
    {
        int[] inputSizes = { 100, 300, 500, 700, 900, 1100, 1300, 1500, 1700, 1900,
        ...
        foreach (int size in inputSizes)
        {
            ...
            for (int i = 0; i < numTests; i++)
            {
                ...
                int[] shortestDistances = DijkstraShortestPath.Dijkstra(startNode, graph);
                sw.Stop();
                ...
                Console.WriteLine($"Input size: {size}, Test case: {i + 1}, Elapsed time: {elapsedMs} ms, Iterations: {graph.Count}");
            }
            ...
            Console.WriteLine($"Average time for input size {size}: {averageTime} ticks, {averageTime / Stopwatch.Frequency} seconds");
